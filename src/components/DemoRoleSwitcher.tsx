...
const DemoRoleSwitcher = () => {
  ...
  const handleRoleSwitch = (account: typeof demoAccounts[0]) => {
    const loggedInUser = login(account.email, account.password, false);
    
    if (loggedInUser) {
      toast({
        title: "Switched to " + account.role,
        description: `Now viewing as ${account.name}`,
      });
      navigate(account.route);
    } else {
      toast({
        title: "Switch failed",
        description: "Could not switch to this role",
        variant: "destructive"
      });
    }
  };
      ...
      <DropdownMenuContent align="end" className="w-56">
        ...
        {demoAccounts.map((account) => (
          <DropdownMenuItem
            ...
            disabled={user?.role === account.role}
          >

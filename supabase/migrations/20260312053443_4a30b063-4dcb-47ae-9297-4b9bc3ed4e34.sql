...
-- =============================================
-- FIX: All portfolio + capstone RLS policies
-- Problem: ALL policies are RESTRICTIVE (Permissive: No)
-- With only restrictive policies, PostgreSQL denies everything.
-- Solution: Drop all and recreate as PERMISSIVE.
-- =============================================
...
CREATE POLICY "Teachers can insert comments" ON public.portfolio_comments FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id AND has_role(auth.uid(), 'teacher'));
...
DROP POLICY IF EXISTS "Students can view own capstones" ON public.capstone_submissions;
DROP POLICY IF EXISTS "Students can insert own capstones" ON public.capstone_submissions;
DROP POLICY IF EXISTS "Students can update own unreviewed capstones" ON public.capstone_submissions;
DROP POLICY IF EXISTS "Teachers can view student capstones" ON public.capstone_submissions;
DROP POLICY IF EXISTS "Teachers can grade capstones" ON public.capstone_submissions;

CREATE POLICY "Students can view own capstones" ON public.capstone_submissions FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Students can insert own capstones" ON public.capstone_submissions FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Students can update own unreviewed capstones" ON public.capstone_submissions FOR UPDATE TO authenticated USING (auth.uid() = user_id AND teacher_score IS NULL) WITH CHECK (auth.uid() = user_id AND teacher_score IS NULL);
CREATE POLICY "Teachers can view student capstones" ON public.capstone_submissions FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM class_members cm JOIN classes c ON c.id = cm.class_id WHERE cm.student_id = capstone_submissions.user_id AND c.teacher_id = auth.uid()));
CREATE POLICY "Teachers can grade capstones" ON public.capstone_submissions FOR UPDATE TO authenticated USING (EXISTS (SELECT 1 FROM class_members cm JOIN classes c ON c.id = cm.class_id WHERE cm.student_id = capstone_submissions.user_id AND c.teacher_id = auth.uid()));
